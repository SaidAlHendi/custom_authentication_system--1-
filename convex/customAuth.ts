...
export const signup = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!existingUser) {
      throw new Error("Registration not allowed. Contact an administrator to create your account first.");
    }
...
// For backward compatibility with existing auth system
export const loggedInUser = query({
  args: {},
  handler: async (ctx) => {
    // This is a placeholder - in the custom system, we use token-based auth
    return null;
  },
});
