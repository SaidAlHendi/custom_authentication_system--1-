...
export default function AdminDashboard({ token }: AdminDashboardProps) {
  ...
  const currentUser = useQuery(api.customAuth.loggedInUser, { token })
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? cleanMessage : 'Failed to create user'
      )
    }
  }
  ...
  const handleUpdateUser = async (user: User) => {
    try {
      ...
      toast.error(
        error instanceof Error ? cleanMessage : 'Failed to update user'
      )
    }
  }
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      ...
      const cleanMessage = getGermanErrorMessage(error)
      toast.error(
        error instanceof Error ? cleanMessage : 'Failed to delete user'
      )
    }
  }
  ...
  const handleResetPassword = async (userId: string) => {
    ...
    try {
      ...
      const cleanMessage = getGermanErrorMessage(error)
      toast.error(
        error instanceof Error ? cleanMessage : 'Failed to reset password'
      )
    }
  }
